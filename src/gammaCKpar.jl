        ...
        for y in  iter_y
            ...
            if dist <= thresh_a
                for ix in indices_x,iy in indices_y
                    results[array_2Dindex(ix,iy),:] = match2
                end
            elseif dist <= thresh_b
                for ix in indices_x,iy in indices_y
                    results[array_2Dindex(ix,iy),:] = match1
                end
            end
        end
    end
...
function gammaCK2par!(vecA::PooledVector,vecB::PooledVector, results::SubArray, array_2Dindex::Function, dims::Tuple;
                     cut_a=0.92, distmethod="jw", w=0.1)
    match2 = [true, true]
    missingval = [false, true]
    # assign distance function
    if distmethod=="jw"
